...
import { Dropdown } from "../customUI/dropdown/Dropdown";
import { DropdownItem } from "../customUI/dropdown/DropdownItem";
...
export default function UserDropdown() {
  ...
  const handleLogout = async () => {
    try {
      // Call your backend admin logout endpoint if needed
      // await fetch('/api/admins/logout', { method: 'POST' })

      // Then sign out from NextAuth
      const data = await signOut({ redirect: false, callbackUrl: '/admin/login' })
  ...
  return (
    <div className="relative">
      ...
      >
        {user?.admin_profile && <span className="mr-3 h-11 w-11 overflow-hidden rounded-full">
          <Image
            width={44}
            height={44}
        ...
        <span className="text-theme-sm mr-1 block font-medium">{user?.admin_name}</span>
      ...
      >
        <div>
          ...
          <span className="text-theme-xs mt-0.5 block text-gray-500 dark:text-gray-400">
            {user?.admin_phone ? user?.admin_phone : "No phone number"}
          </span>
        </div>
